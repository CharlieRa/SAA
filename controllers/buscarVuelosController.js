	...
	function(err, results) {
		...
		for(var i = 0;i < l; i++){
			...
			for(var h = 0;h < results.tcs.length; h++){
				if(results.inf[i].ally_code == results.tcs[h].ALLY_CODE){
					options[i].push( { NAME: results.tcs[h].NAME , ID: results.tcs[h].ID, COST: (results.tcs[h].BASE_COST + results.tcs[h].COST_PER_KM*kms[i]) });
					}
			}
		}
		console.log(options);
		console.log(options[0]);
		res.render('vuelosBuscados', { data: results.inf , citylist: results.cty , opts: options })
	});
	connection.end();
};
